...
\begin{table}[h]
    \begin{tabularx}{\textwidth}{XX}
        \hline
        \textbf{Identified shortcomings} & \textbf{Identified action to resolve the issue} \\
        \hline
        Non-parameterized constructor makes customizable contract deployments impossible & We add a parameterized constructor function to the smart contract, thereby enabling individual deployments of election contract \\
        \hline
        Wrong comparison between expired state and block timestamp & We multiply the block timestamp by 1000 to compare both values in milliseconds \\
        \hline
        \Gls{Admin} wallet is able to vote & To prevent election officials who might know the system \gls{PK} from casting double votes, we wrap the voting function in an if statement checking the transaction's address against the contract's owner \\
        \hline
    \end{tabularx}
...
\begin{table}[h]
    \begin{tabularx}{\textwidth}{XX}
        ...
        We can then inject role guards into routes depending on who should be able to access them. \\
        \hline
        Voter \glsplural{SSN} are not hashed & Hash \glsplural{SSN} with the same package we use to hash passwords. \\
        \hline
        Multiple requests have slow response times & Improve response times by replacing JavaScript in favor of prisma functions that directly manipulate or fetch database objects using SQL commands. \\
        \hline
        Failing voter registration and voting requests & Use transaction object's wait function to await mining of the transaction on the \gls{Blockchain}. \\
        \hline
        Failing test suites & Some \gls{Blockchain} transactions may take longer than the testing framework's default timeout.
        Thus, since transaction times are out of our control, we can only increase the test suite's timeout. \\
        \hline
        Failing concurrent requests & See \cref{subsubsec:res-concurrent-votes} \\
        \hline
    \end{tabularx}
