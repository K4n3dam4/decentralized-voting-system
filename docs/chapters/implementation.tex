...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.8]{boehm-testing-contract}
...
This starts a local \gls{Blockchain} network, creates an admin and a voter wallet.
...
The client application consists of six pages, including pages for administrators (see~\cref{tab:all-pages-accessible-on-the-client-side}).
...
Since we are adhering to atomic design principles (see~\cref{subsec:atomic-design}), a page only fetches data on the server side from our backend services and renders organisms that contain most of our client-side logic.
Consequently, all pages are structured similarly (see listing~\ref{lst:election-page}).

\begin{table}[H]
    \begin{tabularx}{\textwidth}{bCb}
        \hline
        \textbf{Page} & \textbf{Protected (user=\cmark/admin=\dblcmark)} & \textbf{Description} \\
        \hline
        Landing page & \xmark & When users open the voting system they are created by a landing page.
        They will be able to login or register by using the form located on this page. \\
        \hline
        Election overview & \cmark & The page consists of a slider displaying all elections that are currently in the system, \\
        \hline
        ...
        \hline
        Admin overview & \dblcmark & This page displays some analytical information about the voting system and its users.  \\
        \hline
        Admin create panel & \dblcmark & Administrators can create new elections, using the form on this page. \\
        \hline
        Admin election list & \dblcmark & Administrators are provided with an additional page to inspect elections, currently in the system.
        They also have the ability to manually close an election listed on this page.\\
        \hline
    \end{tabularx}
...
\codeFromFile{ts}{../apps/client/pages/election/all/index.tsx}{Election page}{Election page}{lst:election-page}
