...
However, while \gls{POW} requires people to invest in energy, \gls{POS} requires them to deposit funds in the network’s native currency by sending a particular type of transaction~\autocites[321]{antonopoulos_mastering_2019}[3]{udokwu_evaluation_2020}.
...
Shortly after Nakamoto launched Bitcoin, developers started to realize the potential of Nakamoto's vision and began trying to apply the technology to applications in general.
...
Ethereum, as proposed by~\textcite{buterin_ethereum_2014}, combines blockchain technology with a general-purpose computing architecture~\autocite[8]{antonopoulos_mastering_2019}.
Hence, allowing developers \enquote{to create arbitrary consensus-based applications that have the scalability, standardization, feature-completeness, ease of development and interoperability}~\autocite[13]{buterin_ethereum_2014} of regular scripting environments.
This means that developers can program contracts, i.e., smart contracts, whose code is immutably stored on the blockchain and executed by the Ethereum network's virtual machine when parameters specified in the code are met.

Until recently, the Ethereum network was secured by a \gls{POW} consensus mechanism similar to that of Bitcoin.
However, to address the issue of rising energy consumption and scalability, the Ethereum Foundation has long been planning to migrate the network to a \gls{POS} consensus layer.
...
Second-layer blockchains running on Ethereum's mainnet like Polygon are still significantly faster, theoretically scaling up to ~65.000 \gls{TPS}.
