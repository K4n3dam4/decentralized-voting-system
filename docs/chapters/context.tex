...
The Remix \gls{IDE} is an open-source tool for writing, debugging, and deploying smart contracts on supported blockchain networks.
...
Hardhat is a \gls{CLI} tool that provides similar features as the Remix \gls{IDE} (see \cref{subsec:remix-ide}) but is used for local development of smart contracts~\autocite{hardhat_documentation_nodate}.
