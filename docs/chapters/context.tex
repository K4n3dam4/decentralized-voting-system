...
This \gls{Blockchain} was proposed by Vitalik Buterin in~\citeyear{buterin_ethereum_2014} and became known as Ethereum.
Ethereum was the first \gls{Blockchain} offering a general-purpose computing architecture~\autocite[8]{antonopoulos_mastering_2019} with a built-in Turing-complete programming language~\autocite[13]{buterin_ethereum_2014}.
...
Conversely, Solana has the most significant scaling potential due to its \glsfirst{POH} consensus mechanism~\autocite{yakovenko_proof_2020}.
...
More specifically, Hyperledger enables developers to build permissioned \gls{Blockchain} networks where nodes operate under the assumption that less than one-third of their peers are faulty~\autocite[1]{sukhwani_performance_2017}.
